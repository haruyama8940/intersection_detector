...
class deep_learning:
    ...
    def act_and_trains(self, img, intersection_label):
        ...
        if self.first_flag:
            ...
            # self.x_cat_time = torch.tensor(torch.zeros(1,8,3,48,64), dtype=torch.float32, device=self.device) 
            # self.t_cat_time = torch.tensor(torch.zeros(1,8,4), dtype=torch.float32, device=self.device)
            self.x_cat_time = torch.zeros(1,8,3,48,64).to(self.device) 
            self.t_cat_time = torch.zeros(1,8,4).to(self.device)
    ...
    # <to tensor img(x),intersection_label(t)>
        ...
        if self.x_cat.size()[0] == FRAME_SIZE:
            ...
            self.t_cat_time = torch.cat((self.t_cat_time,self.t_cat.unsqueeze(0)),dim=0)
    ...
    # <dataloader>
        ...
        # random_train = random.randint(0,len(train_dataset))
        # print("random:",random_train)
    ...
    # <learning>
        ...
        # loss = self.criterion(y_train, t_label_train[:,-1,:])
        for t in range(0, y_train.size()[0]):

            #print("t:",t,"y_tori = ",y_train[t,:,:].shape,"t_tori =",t_label_train[t,:,:].shape)
            print("leaning")
            self.loss_list= self.criterion(y_train[t,:,:], t_label_train[t,:,:])
            # print("current_loss:",self.loss_list[t].item())
            #loss = self.criterion(y_train[0,:,:], t_label_train[0,:,:])
            ...
            self.train_accuracy += torch.sum(torch.max(y_train[t,:,:], 1)
                                            [1] == torch.max(t_label_train[t,:,:], 1)[1]).item()
            print("accuracy :", self.train_accuracy, "/", BATCH_SIZE*(t+1),
                (self.train_accuracy/len(t_label_train[t,:,:]))*100, "%")
    ...
    # <test>
        ...
        print("intersection_out:",intersection_training[-1,:,:].shape)
        intersection_training_out = torch.max(
            intersection_training[-1,:,:], 1)[1].item()
        ...
        return intersection_training_out, loss_batch
